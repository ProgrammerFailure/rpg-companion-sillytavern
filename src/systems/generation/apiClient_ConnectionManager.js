...
function applyParsedTrackerData(parsedData, { renderUserStats, renderInfoBox, renderThoughts, renderInventory, renderQuests }) {
 ...
 * @param {Function} renderQuests - UI function to render quests
 */

export async function updateRPGData_ConnectionManager(renderUserStats, renderInfoBox, renderThoughts, renderInventory, renderQuests) {
    if (isGenerating) return;
    if (!extensionSettings.enabled) return;
    if (extensionSettings.generationMode !== 'separate') return;
    let RPGUpdatingProfileName = extensionSettings.connectionProfileName;
    ...
    try {
        ...
        if (response) {
            ...
            applyParsedTrackerData(parsedData, { renderUserStats, renderInfoBox, renderThoughts, renderInventory, renderQuests });
        }
    } catch (error) {
